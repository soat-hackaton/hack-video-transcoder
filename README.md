...
- **Tratamento de Logs Avançado e Monitorização (Golden Signals)**: Logs nativamente estruturados com **Context Propagation** utilizando `Correlation IDs` (ou `task_id`) em todos os steps geradores de tarefas, facilitando o rastreamento de problemas. A aplicação também utiliza um middleware customizado para gerar métricas estruturadas compatíveis com *CloudWatch Logs Insights*, suportando três dos quatro Sinais de Ouro de SRE:
  - **Tráfego**: Volume de requisições medido pelo acesso continuado aos endpoints documentados (`path` e `method`).
  - **Latência**: A duração de cada requisição HTTP é capturada nativamente e injetada no atributo numérico `latency_ms` do payload JSON de sucesso ou erro (permitindo o cálculo do percentil P95 ou p99 nos dashboards).
  - **Erros**: As requisições que causam erros de negócio ou falhas internas no processamento lançam nativamente mensagens de erro formatadas no contexto da API, que são capturadas pelo Middleware sob a chave `"error_message"`. Estes logs escalam o seu nível de criticidade a considerar ( `level: WARN/ERROR`).
